...
def main():
    ...
    if args.coordinate:
        ...
        flank = args.flank  # number of bp to take for boundary sequences
        ...
        if vcf_path:
            try:
                ...
                if chr_name in contigs:
                    vcf_chr = chr_name
                elif chr_name.startswith("chr") and chr_name[3:] in contigs:
                    vcf_chr = chr_name[3:]
                elif not chr_name.startswith("chr") and f"chr{chr_name}" in contigs:
                ...
                # Convert variants to DataFrames
                ...
                # Log 5' flank variants
                if five_prime_var_df.empty:
                    log("No 5' flank variants found.")
                else:
                    log(f"5' flank variants:\n{five_prime_var_df}")

                # Log 3' flank variants
                if three_prime_var_df.empty:
                    log("No 3' flank variants found.")
                else:
                    log(f"3' flank variants:\n{three_prime_var_df}")
                ...
                log(f"Error reading VCF contigs or variant lookup: {e}")
        else:
            log(f"No VCF file found for chromosome {chr_name} — skipping variant lookup.")
